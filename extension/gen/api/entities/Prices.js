...
const dao = require("extension/gen/dao/entities/Prices");
const http = require("extension/gen/api/utils/http");
...
rs.service()
	...
	.resource("count/{ItemId}")
	.get(function (ctx, request) {
		let ItemId = parseInt(ctx.pathParameters.ItemId);
		ItemId = isNaN(ItemId) ? ctx.pathParameters.ItemId : ItemId;
		http.sendResponseOk("" + dao.count(ItemId));
	})
	.catch(function (ctx, error) {
		if (error.name === "ForbiddenError") {
			http.sendForbiddenRequest(error.message);
	...
	.get(function (ctx) {
		...
		if (entity) {
			...
			http.sendResponseNotFound("Prices not found");
		}
	})
	...
	.post(function (ctx, request, response) {
		...
		response.setHeader("Content-Location", "/services/js/extension/gen/api/Prices.js/" + entity.Id);
